  ...
  class CodeGenerator
    ...
    def generate_model(world, model, out="")
      ...
      out << "("
        ...
        oc = model.of.const_get(:OBJECT_CONSTRUCTOR__) rescue @encoder.encode_nil
      ...
      #
      unless model.imethods.values.empty?
        out << @encoder.encode_runtime(:add_instance_methods) + "(#{c}, {\n"
        out << model.imethods.values.map {|meth|
          "//\n" + 
          "// #{ meth.name }\n" + 
          "//\n" +
      ...
      # Add class methods 
      #
      unless model.cmethods.values.empty?
        out << @encoder.encode_runtime(:add_class_methods) + "(#{c}, {\n"
        out << model.cmethods.values.map {|meth|
          "//\n" + 
          "// #{ meth.name }\n" + 
          "//\n" +
          @encoder.encode_method(meth.name) + ": " + generate_method(meth)
    ...
    def generate_world(out="")
      ...
      world.entity_models_sorted.each do |model|
        (model.cmethods.values + model.imethods.values).each { |meth|
          meth.node = RubyJS::Compiler.new.sexp_to_node(meth.sexp)
        }
      end 
